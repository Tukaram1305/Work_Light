...
class LEDSY
{
  ...
  LEDSY& operator() (byte tape, byte h, byte s, byte v)
  {
    ...
    {
      case 0:
      { T1R = static_cast<byte>(ceil(RC*255)); T1G = static_cast<byte>(ceil(GC*255)); T1B = static_cast<byte>(ceil(BC*255));
      T1H = h; T1S = s; T1V = v; break; }
      case 1:
      { T2R = static_cast<byte>(ceil(RC*255)); T2G = static_cast<byte>(ceil(GC*255)); T2B = static_cast<byte>(ceil(BC*255));
  ...
  const int cDEL{20};                                     // Opoznienie Kronosa
